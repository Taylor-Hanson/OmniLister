...
serve(async (req) => {
  ...
  try {
    ...
    const mode = url.searchParams.get("mode") ?? "summarized"; // "summarized" | "per-order"
    ...
    // Default GL accounts (can be overridden via mappings)
    const defaults = {
      revenue: "4000",
      shippingIncome: "4001",
      feesExpense: "5000",
    ...
    if (mode === "summarized") {
      ...
      // Per-order journals (feature flagged)
      const sql = `
        select
