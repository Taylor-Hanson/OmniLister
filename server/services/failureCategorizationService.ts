...
export class FailureCategorizationService {
  ...
  private async categorizeByHttpStatus(context: ErrorContext): Promise<FailureAnalysis | null> {
    ...
    return {
      ...
      shouldRetry: categoryConfig.shouldRetry,
      maxRetries: categoryConfig.maxRetries,
      baseDelayMs: categoryConfig.baseDelayMs,
      maxDelayMs: categoryConfig.maxDelayMs,
      backoffMultiplier: Number(categoryConfig.backoffMultiplier),
      jitterRange: Number(categoryConfig.jitterRange),
      requiresUserIntervention: categoryConfig.requiresUserIntervention,
      circuitBreakerEnabled: categoryConfig.circuitBreakerEnabled,
      confidence,
    ...
    if (hasRateLimitHeaders) {
      ...
      if (retryAfter) {
        ...
        if (!isNaN(retryAfterMs)) {
          customDelay = Math.min(retryAfterMs, categoryConfig.maxDelayMs);
        }
      }
  ...
  private getDefaultAnalysis(category: string, context: ErrorContext): FailureAnalysis {
    ...
    return {
      category,
      errorType: "default",
      shouldRetry: defaultCategory.shouldRetry,
      maxRetries: defaultCategory.maxRetries,
      baseDelayMs: defaultCategory.baseDelayMs,
      maxDelayMs: defaultCategory.maxDelayMs,
      backoffMultiplier: Number(defaultCategory.backoffMultiplier),
      jitterRange: Number(defaultCategory.jitterRange),
      requiresUserIntervention: defaultCategory.requiresUserIntervention,
      circuitBreakerEnabled: defaultCategory.circuitBreakerEnabled,
      confidence: 0.3,
